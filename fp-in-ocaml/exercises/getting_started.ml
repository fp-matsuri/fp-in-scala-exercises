...
(** 関数型プログラミング(FP)では関数を取り回すことがよくあるため、名前を付けること *なく* 関数を組み立てるシンタックスがあると便利だ *)
  ...
  let find_first ss key =
    let rec loop n =
      ...
      if n >= Array.length ss then -1
        ...
        (* `ss.(n)`は配列`ss`のn番目の要素を抽出する。 *)
        ss.(n) = key
      then
        (* n番目の要素がキーと等しい場合、nを返す。これは、要素がそのインデックスで配列に現れることを示す。 *)
        n
      else
...
module Polymorphic_functions = struct
  ...
  (** Exercise 2.2: 配列がソート済みかどうかを判定する多相関数を定義せよ。
      第2引数`gt`は配列`arr`の隣接する2要素をとって最初の要素が2番目の要素より大きいかどうかを判定する述語関数。 *)
  let sorted arr gt = failwith "Not implemented"
