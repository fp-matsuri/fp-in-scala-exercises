...
ℹ️ 利用者もコマンドライン実行しやすいようにMakefileやビルド設定ファイルなどにスクリプトとしてまとめておくとよい。

7. [必須] `README.md` に利用者向けの説明を記載する

「使い方」セクションに、REPLがある場合には起動方法と特定のモジュールの試し方、テストや静的解析ツールがある場合には実行方法を記載する。また、「プロジェクト構成」セクションにプロジェクトのルート階層に配置されている主なディレクトリ/ファイルについて簡単な説明を示す(`tree` コマンドの出力をコメント補足するとよいかも)。
