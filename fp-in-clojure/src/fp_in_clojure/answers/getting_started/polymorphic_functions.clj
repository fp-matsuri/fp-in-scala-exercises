(ns fp-in-clojure.answers.getting-started.polymorphic-functions
...
(defn compose [f g]
  ;; NOTE: `->` (thread-first)マクロにより関数適用の連鎖を簡潔に表現できる(ここでは `(f (g a))` と等価)
  ;; ref. https://clojuredocs.org/clojure.core/-%3E
  (fn [a] (-> a g f)))
