(ns fp-in-clojure.answers.state.rng
...
(defn double []
  (fn [rng]
    (let [[i rng'] ((non-negative-int) rng)]
      [(/ i (-> Integer/MAX_VALUE inc core/double)) rng'])))
...
(defn -double []
  (map (fn [i] (/ i (-> Integer/MAX_VALUE inc core/double)))
       (non-negative-int)))
...
(defn sequence [rs]
  (reduce (fn [acc r] (map2 cons r acc)) (unit ()) (reverse rs)))
